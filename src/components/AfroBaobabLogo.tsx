...
export default function AfroBaobabLogo({
  ...
  const renderEmblem = (sizeClass: string) => (
    ...
    >
      ...
      <circle cx="100" cy="80" r="7.5" fill={color} /> {/* Main Center Head */}
      
      <circle cx="78" cy="86" r="6.5" fill={color} /> {/* High Left Head */}
      <circle cx="122" cy="86" r="6.5" fill={color} /> {/* High Right Head */}

      <circle cx="58" cy="94" r="6" fill={color} /> {/* Outer High Left Head */}
      <circle cx="142" cy="94" r="6" fill={color} /> {/* Outer High Right Head */}

      <circle cx="42" cy="112" r="6" fill={color} /> {/* Left Mid Head */}
      <circle cx="158" cy="112" r="6" fill={color} /> {/* Right Mid Head */}

      <circle cx="34" cy="138" r="5.5" fill={color} /> {/* Outer Left Head */}
      <circle cx="166" cy="138" r="5.5" fill={color} /> {/* Outer Right Head */}

      <circle cx="38" cy="164" r="5.5" fill={color} /> {/* Lower Outer Left Head */}
      <circle cx="162" cy="164" r="5.5" fill={color} /> {/* Lower Outer Right Head */}
  ...
  if (variant === "navbar") {
    return (
      <div className={`flex items-center gap-3.5 ${className}`}>
        {renderEmblem("h-11 sm:h-13 w-auto")}
        <div className="flex flex-col select-none text-white">
          <div className="flex items-center gap-1.5 font-sans uppercase font-bold text-base tracking-[0.2em] leading-none">
            AFRO BAOBAB
          </div>
          <div className="text-[0.45rem] tracking-[0.3em] font-sans font-semibold text-clay mt-1 whitespace-nowrap">
            CULTURAL HUB &amp; ART GALLERY
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`flex items-center gap-4 ${className}`}>
      {renderEmblem("h-16 md:h-20 w-auto")}
      {renderTypography()}
    </div>
  );
}
